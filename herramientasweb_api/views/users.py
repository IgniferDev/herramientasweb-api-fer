...
class UsersAll(generics.CreateAPIView):
    permission_classes = (permissions.IsAuthenticated,) #no puede acceder a la base si no esta registrado
...
class UsersViewEdit(generics.CreateAPIView):
    ...
    def put(self, request, *args, **kwargs):
        ...
        temp.save()
        user = ProfilesSerializer(profile, many=False).data#para dar estructura en formato json
        return Response(user,200)
    
    def delete(self, request, *args, **kwargs):
        profile = get_object_or_404(Profiles, id=request.GET.get("id"))#GETget es el que cacha el parametro en django
        try:
            profile.user.delete()
    ...
    #IMPORTANTE TENER CUIDADO CON LA IDENTACION O TABS EN PYTHON SINO NO JALA CONFIRMADO
    #CAUTION CUIDADOx   
